...
### Параметры WindowDefinition
| Параметр | Тип | Описание |
|---|---|---|
| `windowId` | string | Уникальный идентификатор экрана. Используется в командах `screen:windowId` |
| `backgroundColor` | string | Цвет фона экрана в формате HEX. Например: `"#1D1D1E"` |
| `onOpened` | string, object или array | Команда, выполняемая **однократно** при первом открытии экрана (переходе на него) |
| `onFocused` | string, object или array | Команда, выполняемая **каждый раз**, когда экран получает фокус — в том числе при возврате назад через `back` |
| `isDefaultLoader` | bool | Если `true` — экран используется как экран загрузки. Показывается автоматически во время выполнения синхронных API-запросов |
| `overlay` | OverlayConfig | Конфигурация оверлея. Если задан — экран отображается поверх основного контента |
| `elements` | ElementDefinition[] | Массив визуальных элементов экрана |

> Разница между `onOpened` и `onFocused`: `onOpened` срабатывает только при прямом переходе на экран (`screen:`), `onFocused` — при каждом получении фокуса, включая возврат через `back`.

### Параметры OverlayConfig
Оверлей — экран, постоянно отображаемый поверх других. Настраивается в параметре `overlay` окна.

| Параметр | Тип | Описание |
|---|---|---|
| `AutoShow` | bool | Если `true` — оверлей показывается автоматически на всех экранах из списка `EnableFor` |
| `ZIndex` | int | Порядок наложения. Больше — выше. Рекомендуется `150` для навигационных оверлеев |
| `EnableFor` | string[] | Список `windowId`, на которых оверлей отображается автоматически |

```json
{
  "windowId": "win_NavigationOverlay",
  "backgroundColor": "#00000000",
  "overlay": {
    "AutoShow": true,
    "ZIndex": 150,
    "EnableFor": ["win_QRInput", "win_ManualInput", "win_CodeInput"]
  },
  "elements": [
    {
      "type": "button",
...
Управление оверлеями вручную через команды:
```json
"showOverlay:win_NavigationOverlay"
"hideOverlay:win_NavigationOverlay"
"toggleOverlay:win_NavigationOverlay"
```
---
...
Параметр `translationKey` поддерживает не только ключи локализации, но и прямой текст (например, эмодзи):
...
```
Если ключ не найден в таблице переводов — значение используется как есть.